...
class DatabasePersistencePropertyTest {
    ...
    private fun appRuleArb(appInfoId: Long) = arbitrary {
        ...
        AppRuleEntity(
            ...
            timeRangeStart = String.format(
                "%02d:%02d",
                startHour,
                startMinute
            ),
            timeRangeEnd = String.format(
                "%02d:%02d",
                endHour,
                endMinute
            ),
    ...
    @Test
    fun propertyTest_AppInfoPersistenceRoundTrip() {
        runBlocking {
            checkAll(
                100,
                appInfoArb()
    ...
    @Test
    fun propertyTest_AppRulePersistenceRoundTrip() {
        runBlocking {
            checkAll(
                100,
                appInfoArb()
                ...
                checkAll(
                    ...
                    assertEquals(
                        appRule.timeRangeStart,
                        retrieved.timeRangeStart
                    )
                    assertEquals(
                        appRule.timeRangeEnd,
                        retrieved.timeRangeEnd
                    )
                    assertEquals(
                        appRule.totalTime,
                        retrieved.totalTime
